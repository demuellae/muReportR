...
setMethod("initialize", "ReportPlot",
	function(.Object, fname="temp", report=NULL, width=7, height=7, create.pdf=FALSE, low.png=0L, high.png=0L, skip.dev.setup=FALSE) {
		...
		if (!is.null(report)) {
			.Object@dir.pdf <- getReportDir(report, "pdfs", absolute = TRUE)
			.Object@dir.png.low <- getReportDir(report, "images", absolute = TRUE)
			.Object@dir.png.high <- getReportDir(report, "images_highres", absolute = TRUE)
		} else {
...
#'
#' @param repPlot Report plot of interest. This must be an object of type \code{\linkS4class{ReportPlot}}.
...
getReportPlotFiles <- function(repPlot) {
	if (!inherits(repPlot, "ReportPlot")) {
		stop("invalid value for repPlot")
	}
	result <- character(0)
	fname <- repPlot@fname
	if (repPlot@create.pdf) {
		result <- file.path(repPlot@dir.pdf, paste(fname, "pdf", sep = "."))
	}
	if (repPlot@low.png > 0) {
		result <- c(result, file.path(repPlot@dir.png.low, paste(fname, "png", sep = ".")))
	}
	if (repPlot@high.png > 0) {
		if (repPlot@low.png && repPlot@dir.png.low == repPlot@dir.png.high) {
			fname <- paste(fname, "_highres", sep = "")
		}
		result <- c(result, file.path(repPlot@dir.png.high, paste(fname, "png", sep = ".")))
	}
	return(result)
}
